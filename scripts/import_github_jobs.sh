...
block_issue() {
  ...
  if [[ ! -f "${blocked_marker}" ]]; then
    ...
    post_issue_state "${issue_number}" "BLOCKED" "Reason: ${reason}\nRepo Name: ${repo_name}\nRecovery: register the workspace, then run requeue_github_issue.sh ${issue_number}." || true
  fi
}
...
${issue_body}"; then
  ...
  if workspace_required "${job_type}" && [[ ! -r "${REGISTRY_DIR}/${repo_name}.env" ]]; then
    block_issue "${issue_number}" "${issue_author}" "WORKSPACE_NOT_REGISTERED" "${repo_name}"
