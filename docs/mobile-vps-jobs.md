...
2. open `Issues`
3. tap `New issue`
4. choose `VPS Codex Direct` for one short instruction to Codex only, `VPS Free Request` for natural-language planning, or `VPS Job Casual` for a known short request
...
Use `VPS Codex Direct` when the VPS should run the authenticated Codex CLI and may edit files in the registered workspace. This is the normal smartphone lane when you want Codex only, not an AI会議室 style discussion. Use `VPS Quick AI Exec` or the longer `VPS AI Exec` form only when you prefer those older names. These routes do not run `git push`, create PRs, or create secrets.
...
## Fastest Useful Requests

Use `VPS Codex Direct` for one short task:

```text
READMEとdocs/vps-ai-cli-runner.mdを確認して、編集せずに現状を報告して。
```

```text
scripts/*.shのbash構文だけ確認して、結果を報告して。
```

```text
次に安全に進める作業を1つだけ選んで、理由と確認方法を報告して。
```

Use `VPS Free Request` when you want planning only and no file edits:
...
- request says `ai-councilの状態見て`, `ai-councilをチェックして`, `ai-councilを検証して`, `作業場まとめて`, uses direct `JOB_TYPE` lines, uses `VPS Codex Direct`, uses `VPS Quick AI Exec`, uses `VPS AI Exec`, or uses a free-form planning request
